# VetBooking - Documentação do Backend
...
## Visão Geral
O backend do VetBooking é construído em Python utilizando o framework Flask, com um banco de dados MySQL para persistência dos dados. O sistema gerencia o agendamento de consultas veterinárias, cadastro de usuários (tutores, veterinários e clínicas) e gerenciamento de pets.
...
- `tipo` (VARCHAR): Tipo de usuário (tutor, veterinario, clinica)
...
- `crmv` (VARCHAR): Número do CRMV
- `especialidades` (TEXT): Especialidades do veterinário
...
- `medico_id` (INT): ID do veterinário
...
- `GET /medicos/<id>`: Obtém detalhes do veterinário
- `PUT /medicos/<id>`: Atualiza informações do veterinário
...
- `POST /consultas`: Agenda nova consulta
  - Corpo: `{"pet_id": "int", "medico_id": "int", "clinica_id": "int", "data_hora": "datetime"}`
